...
export const api = {
  ...
  createRun: () =>
    fetch("/api/runs", { method: "POST" }).then(parseJson),
  startSample: (body) =>
    fetch("/api/runs/sample", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body || { profile_id: "adult", sdh_mode: true, has_ad: true }),
    }).then(parseJson),
  startRun: (id, body) =>
  ...
  decideFix: (runId, fixId, decision) =>
    fetch(`/api/runs/${runId}/fixes/${fixId}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ decision }),
    }).then(parseJson),
  uploadAssets: async (id, captionFile, adFile) => {
    const form = new FormData();
    if (captionFile) form.append("captions", captionFile);
    if (adFile) form.append("ad", adFile);
    const res = await fetch(`/api/runs/${id}/assets`, { method: "POST", body: form });
